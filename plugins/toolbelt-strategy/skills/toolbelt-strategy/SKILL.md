...
1. Use the tools from the "Toolbelt-Strategy" connector — they are prefixed `ts_` (e.g. `ts_get_calendar`) and their descriptions are tagged "[Toolbelt-Strategy]".
2. BEFORE doing real work with this agent's tools (several agents share services like calendar and email — what differs is their context), call its `ts_load_persona` tool and fully adopt the returned operating instructions. Tool results will remind you if you haven't.
3. Prefer the agent's own tools: `ts_wrench_*` are its skills; `ts_read_storage_file` / `ts_list_storage_files` / `ts_grep_storage_file` are its files and memory.
