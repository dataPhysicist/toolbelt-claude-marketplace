...
#
# Required env: AGENT_NAME, WORKSPACE_ID
...
: "${AGENT_NAME:?set AGENT_NAME}"; : "${WORKSPACE_ID:?set WORKSPACE_ID}"
...
# Assemble manifest from the template: stamp name/desc/prefix/workspace, drop the
# user_config fields we bake, set the prefixed persona prompt text.
...
const name=process.env.AGENT_NAME, ws=process.env.WORKSPACE_ID, prefix=process.env.PREFIX, desc=process.env.DESC, slug=process.env.SLUG, base=process.env.BASE_URL;
m.name="apexti-"+slug; m.display_name=name; m.description=desc;
m.server.mcp_config.env.TOOLBELT_ASSISTANT_NAME=name;
m.server.mcp_config.env.TOOLBELT_WORKSPACE_ID=ws;
m.server.mcp_config.env.TOOLBELT_TOOL_PREFIX=prefix;
m.server.mcp_config.env.TOOLBELT_BASE_URL=base;
delete m.user_config.assistant_name; delete m.user_config.workspace_id;
